      ...
      .then(async () => {
        await User.find().then((users) => {
          users.forEach(async (user) => {
            await sendEmail(user.email, `new book :${title}`).catch(
              console.error
            );
          });
        });
...
export const getStats = async (req, res) => {
  const lecteur_number = await User.find({ role: "lecteur" });
  const borrow_number = await emprunt.find({});
  const book = await Book.find({}, "title author").sort({
    borrowing_number: "desc",
  });
  res.json({
    "Readers number": lecteur_number.length,
    "Borrows number": borrow_number.length,
    book,
  });
};

export const getlecteurHistory = async (req, res) => {
  const books = await emprunt
    .find({ lecteur: res.locals.user_id }, "stat createdAt book")
    .populate("book", "title")
    .catch((err) => {
      res.status(500).json({ error: err });
    });
...
export const replyComment = async (req, res) => {
  ...
  await Reply.create({ reply, comment: res.locals.user_id })
    .then(() => {
...
export const editReplyComment = async (req, res) => {
  const { reply } = req.body;
  await Reply.findByIdAndUpdate(req.params.id, { reply })
    .then((result) => {
      if (result) {
        res.json("comment updated Successfully !");
      } else {
...
export const renewBook = async (req, res) => {
  await emprunt.findById(req.params.id).then(async (result) => {
    if (!result.renewed) {
      result.renewed = true;
      ...
      res.json({
        message:
          "you already renewed it once, consider returning it then borrow it again",
      });
    }
  });
};
